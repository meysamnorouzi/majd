      ...
      <Container>
        ...
        >
          {courseFormats.map((format) => {
            ...
            return (
              <StaggerItem key={format.slug} variant="up">
                <Link
                  href={`/courses/${format.slug}/`}
                  className="flex h-full items-center gap-4 rounded-xl border border-slate-100 bg-cream/30 p-5 transition hover:border-gold-500/30 hover:shadow-md"
                >
                  <div
